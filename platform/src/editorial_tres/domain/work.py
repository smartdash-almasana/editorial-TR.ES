...
class Work(BaseModel):
    """Immutable aggregate with global stream and material manuscript versions."""

    tenant_id: TenantId; editorial_id: EditorialId; work_id: WorkId; title: str; language: str; status: str = "conceived"; version: int = Field(default=1, ge=1); manuscript_version: int = Field(default=1, ge=1)
    ...
    @classmethod
    def _default_manuscript_version(cls, data):
        if isinstance(data, dict) and "manuscript_version" not in data:
            normalized = dict(data)
            normalized["manuscript_version"] = normalized.get("version", 1)
            return normalized
        return data
    ...
    def _same_owner(self):
        ...
        return self
    @model_validator(mode="after")
    def _version_order(self):
        if self.manuscript_version > self.version:
            raise ValueError("La revisión material no puede superar la versión global del stream.")
        return self
    @classmethod
    ...
    def apply(self, event: DomainEvent) -> "Work":
        ...
        if event.event_type == "content_block.added":
            expression_graph = expression_graph.add_block(ContentBlock.model_validate(event.payload["block"]))
